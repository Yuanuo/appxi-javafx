 ...
 *
 * @author Jens Deters
 * @param <T> The type of GlyphEnum enum.
 */
public abstract class GlyphIcon<T extends Enum<T> & GlyphEnum> extends Text {
    ...
    void updateIcon() {
        GlyphEnum icon = getDefaultGlyph();
        try {
            ...
            LoggerFactory.getLogger(GlyphIcon.class.getName()).warn(msg);
        }
    ...
    static abstract class GlyphView<T extends Enum<T> & GlyphEnum> extends GlyphIcon<T> {

        public GlyphView(T icon, String iconSize) {
            super((Class<T>) icon.getClass());
            setIcon(icon);
            setStyle(String.format("-fx-font-family: %s; -fx-font-size: %s;", icon.fontFamily(), iconSize));
        }

        public GlyphView(T icon) {
            this(icon, "1em");
        }
    }
}
