    ...
    public TabPaneEx() {
        ...
        tabsChangeListener = change -> {
            if (null != change && change.next() && change.wasAdded()) {
                this.lookupAll(".tab-header-area .headers-region .tab .tab-container .tab-close-button")
                        .forEach(btn -> {
                            if (btn.getProperties().containsKey(AK_HACKED))
                                return;
                            ...
                            btn.setOnMousePressed(evt -> {
                                ...
                                if (btn != evt.getPickResult().getIntersectedNode())
                                    return;
                                evt.consume();
                            });
                            btn.setOnMouseReleased(evt -> {
                                ...
                                if (btn != evt.getPickResult().getIntersectedNode())
                                    return;
                                EventHandler<? super MouseEvent> originHandle =
                                        (EventHandler<? super MouseEvent>) btn.getProperties().get(AK_HANDLE);
                                if (null != originHandle)
                                    originHandle.handle(evt);
                            });
                        });
            }
        };
