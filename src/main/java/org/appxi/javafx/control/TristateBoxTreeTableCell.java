 ...
 * {@link #TristateBoxTreeTableCell(Callback, StringConverter)} constructor.
 *
 ...
 * @since JavaFX 8.0
 */
public class TristateBoxTreeTableCell<S, T> extends TreeTableCell<S, T> implements TristateConstants {
     ...
     */
    public static <S> Callback<TreeTableColumn<S, Object>, TreeTableCell<S, Object>> forTreeTableColumn(
    ...
    public static <S, T> Callback<TreeTableColumn<S, T>, TreeTableCell<S, T>> forTreeTableColumn(
            final Callback<Integer, ObservableValue<Object>> getSelectedProperty) {
    ...
    public static <S, T> Callback<TreeTableColumn<S, T>, TreeTableCell<S, T>> forTreeTableColumn(
        ...
        StringConverter<T> converter = !showLabel ? null : (StringConverter<T>) defaultStringConverter;
    ...
    public static <S, T> Callback<TreeTableColumn<S, T>, TreeTableCell<S, T>> forTreeTableColumn(
        ...
        return list -> new TristateBoxTreeTableCell<>(getSelectedProperty, converter);
    }
    ...
    private ObservableValue<?> cachedProperty;

    private ChangeListener<? super Boolean> objectPropertySetter;
     ...
     */
    public TristateBoxTreeTableCell() {
        this(null, null);
    }
...
//        prop.set(this, Pos.CENTER);
    ...
    private final ObjectProperty<Callback<Integer, ObservableValue<Object>>> selectedStateCallback = new SimpleObjectProperty<>(this, "selectedStateCallback");
    ...
    public void updateItem(T item, boolean empty) {
        ...
        if (objectPropertySetter != null) {
            checkBox.selectedProperty().removeListener(objectPropertySetter);
            objectPropertySetter = null;
        }
        if (cachedProperty instanceof BooleanProperty property) {
        ...
        if (empty) {
            ...
            if (obsValue == null || obsValue.getValue() == null) {
                ...
                cachedProperty = obsValue;
                if (obsValue instanceof BooleanProperty property) {
                    checkBox.selectedProperty().bindBidirectional(property);
                } else if (obsValue instanceof Property property) {
                    checkBox.setSelected(TristateConstants.isSelected(property.getValue()));
                    objectPropertySetter = (obs, oldVal, newVal) -> property.setValue(newVal);
                    checkBox.selectedProperty().addListener(objectPropertySetter);
                } else {
                    checkBox.setSelected(TristateConstants.isSelected(obsValue.getValue()));
                }
